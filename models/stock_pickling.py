...
class Inventory_picking(models.Model):
    ...
    def cancel_stock_picking(self):
        for move in self.move_ids_without_package:
            if move.product_id.type == 'product':
                ...
                if move.picking_id.picking_type_id.code == 'outgoing':
                    ...
                    if move.product_id.tracking == 'none':
                        ...
                        #    [('product_id', '=', move.product_id.id), ('location_id', '=', move.location_id.id)], limit=1)

                        # quant._update_available_quantity(
                        #    move.product_id, move.location_id, move.quantity_done)
                        ...
                        # quant = self.env['stock.quant'].search([('product_id', '=', move.product_id.id), (
                        #    'location_id', '=', move.location_id.id), ('lot_id', '=', lot_id.id)], limit=1)
                        # quant._update_available_quantity(
                        # move.product_id, move.location_id, move.quantity_done, lot_id)
                    ...
                    if move.product_id.tracking == 'none':
                        quant = self.env['stock.quant'].search(
                            [('product_id', '=', move.product_id.id), ('location_id', '=', move.location_dest_id.id)], limit=1)
                        quant._update_available_quantity(
                            move.product_id, move.location_dest_id, -move.quantity_done)
                    else:
                        quant = self.env['stock.quant'].search([('product_id', '=', move.product_id.id), (
                            'location_id', '=', move.location_dest_id.id), ('lot_id', '=', lot_id.id)], limit=1)

                        quant._update_available_quantity(
                            move.product_id, move.location_dest_id, -move.quantity_done, lot_id)
            ...
            journal_rec = self.env['account.move'].sudo().search(
                [('stock_move_id', '=', move.id)], order="id desc", limit=1)
            journal_rec.button_cancel()
            journal_rec.sudo().unlink()
            move.write({'state': 'draft'})
            move.move_line_ids.with_context(force=True).unlink()
            move.sudo().unlink()
...
class stock_move_line(models.Model):
    ...
    def unlink(self):
            ...
            if ml.product_id.type == 'product' and not ml.location_id.should_bypass_reservation() and not float_is_zero(ml.product_qty, precision_digits=precision):
                try:
                    self.env['stock.quant']._update_reserved_quantity(
                        ml.product_id, ml.location_id, -ml.product_qty, lot_id=ml.lot_id, package_id=ml.package_id, owner_id=ml.owner_id, strict=True)
                except UserError:
                    if ml.lot_id:
                        self.env['stock.quant']._update_reserved_quantity(
        ...
        if self._context.get('force') == True:
            res = super(stock_move_line, self).unlink()
            return res
